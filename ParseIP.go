...
func ParseIP(host string, filename string, nohosts ...string) (hosts []string, HostPort []string, err error) {
	...
	if filename != "" {
		var filehost []string
		filehost, HostPort, _ = Readipfile(filename)
...
func parseIP1(ip string) []string {
	...
	testIP := net.ParseIP(IPRange[0]).To4()
	var AllIP []string
	if len(IPRange[1]) < 4 {
